...
from ltr.models.scheduler.CosineAnnealing import CosineAnnealingLr
...
def run(settings):
    ...
    settings.env.workspace_dir = '/home/xlsun/xlsun/code/TransT/results/SwinTransT_Cvt3_coslr'
    ...
    lr_scheduler = CosineAnnealingLr(optimizer, max_epoch=500, warmup_epoch=20, warmup_ratio=0.1, min_lr=1e-5)

    # Create trainer
    trainer = LTRTrainer(actor, [loader_train], optimizer, settings, lr_scheduler)

    # Run training (set fail_safe=False if you are debugging)
    trainer.train(500, load_latest=False, fail_safe=False)
