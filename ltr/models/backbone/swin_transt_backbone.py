...
class SwintranstCvtDiffcnn_Backbone(BackboneBase):
    """ResNet backbone with frozen BatchNorm."""
    def __init__(self,
                 output_layers,
                 frozen_stages,
                 pretrained_model_path,
                 **params):
        backbone = backbones.SwinTransformer_cvt_diffcnn(out_indices=output_layers, frozen_stages=frozen_stages, **params)
        backbone.init_weights(pretrained_model_path)
        num_channels = None
        super().__init__(backbone, num_channels)
...
def build_swin_transformer_cvt_diffcnn_backbone(settings, name, load_pretrained=False, output_layers=(3,), frozen_stages=-1,
                                    overwrite_embed_dim=None):
    import copy
    from .swin_transformer_qkvcnn import _cfg
    ...
    position_embedding = build_position_encoding(settings)
    backbone = SwintranstCvtDiffcnn_Backbone(output_layers, frozen_stages, pretrained_model_path, **params)
    backbone.num_channels = params['embed_dim'] * 2 ** (max(output_layers))
    model = Joiner(backbone, position_embedding)
    model.num_channels = backbone.num_channels
