...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix=None):
        ...
        if H == W and region_type == 'search'and H in [64, 32, 16]:
            x = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
            x = self.dwconv_block_search(x)
            x = x.permute(0, 2, 3, 1).flatten(1, 2)
        
        elif H == W and region_type == 'template'and H in [32, 16, 8]:
            x = x.reshape(B, H, W, C).permute(0, 3, 1, 2)
...
class SwinTransformer_cvt_diffcnn(nn.Module):
    def __init__(self,
                 pretrain_img_size=224,
                 patch_size=4,
                 in_chans=3,
                 ...
                 frozen_stages=-1,
                 use_checkpoint=False):
        super(SwinTransformer_cvt_diffcnn, self).__init__()
    ...
    def forward(self, x, out_indices=None, reshape=True):
        ...
        _, _, H, W = x.size()
        global region_type
        region_type = 'search' if H == 256 else 'template'
        outs = OrderedDict()
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer_cvt_diffcnn, self).train(mode)
        _freeze_stages(self.stages, self.frozen_stages, self.ape)
...
def build_swin_transformer_backbone(name, load_pretrained=True, output_layers=(2,), frozen_stages=-1,
    ...
    if overwrite_embed_dim is not None:
        ...
        pretrained_model_path = None
    transformer = SwinTransformer_cvt_diffcnn(out_indices=output_layers, frozen_stages=frozen_stages, **params)
