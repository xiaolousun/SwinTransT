...
from ltr.models.backbone.swin_transt_backbone import build_swin_transformer_cvt_diffcnn_backbone
...
class SwinTransT(nn.Module):
    ...
    def track(self, search):
        ...
        # import os
        # reg_cls_feat_reshape = hs.flatten(0, 2).reshape(32, 32, -1).detach().cpu().numpy()
        # C = reg_cls_feat_reshape.shape[-1]
        # save_path = '/home/xlsun/xlsun/code/TransT/results/save_fig_transt'
        # if not os.path.exists(save_path):
        #     os.makedirs(save_path)

        # for i in range(C):
        #     fig = reg_cls_feat_reshape[:, :, i]
        #     plt.imshow(fig)
        #     plt.savefig(save_path+'/'+'pic-{}.png'.format(i + 1))
...
def transt_swintranst(settings):
    ...
    backbone_net, output_layer = build_swin_transformer_cvt_diffcnn_backbone(settings, name, output_layers=(2,), load_pretrained=False)
