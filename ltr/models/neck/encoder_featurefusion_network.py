...
class TransformerEncoderFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        feat = torch.cat([src1, src2], dim=0)
        feat_mask = torch.cat([src1_mask, src2_mask], dim=1)
        pos_embed = torch.cat([pos_src1, pos_src2], dim=0)

        q = k = self.with_pos_embed(feat, pos_embed)  # add pos to src
        src2 = self.self_attn3(q, k, feat, attn_mask=feat_mask,
                              key_padding_mask=None)[0]
        src = src + self.dropout31(src2)
        src = self.norm31(src)
...
def build_backbone_enhance_network(params, settings, output_layer):
    return Transformer_Enhance(
        d_model=params['embed_dim'] * 2 ** (output_layer),
        dropout=0.1,
        nhead=params['num_heads'][-1], 
        dim_feedforward=(params['embed_dim'] * 2 ** (output_layer))*4,
...
def build_encoder_featurefusion_enhance_network(params, settings, output_layer):
